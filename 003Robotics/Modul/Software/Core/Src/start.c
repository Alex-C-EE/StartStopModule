...
void start_state_changed_notify(void);
...
/*
 * Call this function at 100Hz (from SysTick)
 */
	...
	if (start_state == START_STATE_STOPPED_SAFE) {
		if (time) {
			...
			if (!time) {
				...
				start_save_data.state = START_STATE_POWER_ON; // Ready for next power-on
...
char start_init(void(*save_func)(START_DATA *data),
    ...
    // If state was STOPPED, reset to POWER_ON after loading
    if (start_save_data.state == START_STATE_STOPPED || start_save_data.state == START_STATE_STOPPED_SAFE) {
        start_save_data.state = START_STATE_POWER_ON;
    }
 ...
 */
void start_irdata_handler(IRDATA *data) {
	if (start_state == START_STATE_STOPPED) { // Note: We still listen during STOPPED_SAFE
		return;
	}
	...
	if (data->address_low == START_ADDRESS_PROG) {
		...
		switch (start_state) { // Use 'start_state' for current state
		case START_STATE_POWER_ON:
			if (start_comp_mode) {
				if (data->address_low == START_ADDRESS_COMP) {
		...
		case START_STATE_STOPPED_SAFE: // Allow re-stop command during safe stop
			if (start_comp_mode) {
				if (data->address_low == START_ADDRESS_COMP) {
        ...
        case START_STATE_STOPPED:
            // Do nothing, we are hard-stopped
            break;
		}
	}
}
